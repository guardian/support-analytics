...
describe('build-query functions', () => {
	...
	describe('buildTestSpecificQuery', () => {
		it('should build a query for test specific data', () => {
			const mockPricingCaseStatement = `CASE product
        WHEN 'SUPPORTER_PLUS' THEN 10
        WHEN 'DIGITAL_SUBSCRIPTION' THEN 20
        ...
        END`;
			...
			expect(query).toContain(mockPricingCaseStatement);
		});

		it('should use the provided pricing CASE statement', () => {
			const customPricingCaseStatement = `CASE product
        WHEN 'TEST_PRODUCT' THEN 99.99
        END`;
			...
			const query = buildTestSpecificQuery(
				...
				customPricingCaseStatement,
			);

			expect(query).toContain("WHEN 'TEST_PRODUCT' THEN 99.99");
		});
	});
});
